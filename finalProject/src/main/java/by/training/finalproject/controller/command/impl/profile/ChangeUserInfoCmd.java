...
public class ChangeUserInfoCmd extends Command {
    private static Logger logger = LogManager.getLogger(ChangeUserInfoCmd.class.getName());
    ...
    public Forward execute(HttpServletRequest request, HttpServletResponse response, ServiceFactory factory) {
        ...
        try {
            if (UserUtil.loadUser(request, factory, session)) {
                return new Forward("/profile/changeUserInfo.jsp", false);
            }
            return forward;
        } catch (ServiceException e) {
            logger.error("Failed to load change profile page", e);
            return forward;
        }
    }
